...
struct counter_synapse {
	size_t					action;
	const enum synapse_action *const	actions_list;
	size_t					num_actions;
...
struct counter_count {
	...
	size_t					function;
	const enum count_function *const	functions_list;
 ...
 *			should be set only via the set_signal_read_val function.
 * @count_read:		optional read callback for Count attribute. The read
 *			value of the respective Count should be passed back via
 *			the val parameter. val points to an opaque type which
 *			should be set only via the set_count_read_val function.
